"""
scrapers/sources/killians.py

Killian's, Spearfish SD — beers on tap via Untappd venue menu.
https://untappd.com/v/killians/556648

Untappd renders the full menu server-side, so a plain GET with a browser
User-Agent returns structured HTML. Each beer is an `li.menu-item` with:
  - .beer-label img   → label image (https://labels.untappd.com/{beer_id})
  - .beer-details h5 a → name + per-beer Untappd URL
  - .beer-details h5 em → style/category
  - .beer-details h6 span → "X% ABV • Y IBU • Brewery"
  - [data-href=":brewery"] → brewery name
  - .num → Untappd rating
"""
...
SOURCE_URL = "https://untappd.com/v/killians/556648"
_STATS_RE = re.compile(r"([\d.]+)%\s*ABV(?:\s*•\s*([\d.]+)\s*IBU)?", re.IGNORECASE)


class Killians(BaseScraper):
    name = "Killian's"
    slug = "killians"
    dedup_key = "slug"
    replace = True
